...
instance monadCanvasActionRun ∷ (TypeEquals (Proxy r1) (Proxy (CTX + EFFECT + r2))) ⇒ MonadCanvasAction (Run r1) where
...
else instance monadCanvasAffReaderT ∷ MonadCanvasAff m ⇒ MonadCanvasAff (ReaderT r m) where
  liftCanvasAff = lift <<< liftCanvasAff


instance monadCanvasAffRun ∷ (TypeEquals (Proxy r1) (Proxy (CTX + AFF + EFFECT + r2))) ⇒ MonadCanvasAff (Run r1) where
...
instance monadCanvasAffRWST ∷ (Monoid w, MonadCanvasAff m) ⇒ MonadCanvasAff (RWST r w s m) where
  liftCanvasAff = lift <<< liftCanvasAff

instance monadCanvasAffStateT ∷ MonadCanvasAff m ⇒ MonadCanvasAff (StateT s m) where
  liftCanvasAff = lift <<< liftCanvasAff

instance monadCanvasAffWriterT ∷ (Monoid w, MonadCanvasAff m) ⇒ MonadCanvasAff (WriterT w m) where
  liftCanvasAff = lift <<< liftCanvasAff
